...
| GET | `/v1/push/config` | 已实现（通道第一批） | Web Push 功能状态与 VAPID 公钥 |
...
| POST | `/v1/notifications/read-all` | 已实现（通知第五批） | 幂等地将当前用户全部站内通知标记已读 |
...
| `0023_create_push_subscriptions` | 创建加密存储的 PWA Web Push 终端表 |
| `0024_create_notification_pipeline` | 创建提醒、偏好、Outbox、通知任务、站内通知与终端投递表 |
...
| MCP Apps UI | Server `app/modules/mcp/server.py` 轻量壳；源码/静态 bundle 位于 `MomentOneWeb/src/mcp-apps/` 与 `/mcp-apps/v1/` | 已实现 4 类响应式结果卡：`bookkeeping` / `timeline` / `habits` / `utility`；17 个面向用户的 MCP 查询和写入结果均绑定独立 UI，3 个附件传输工具保持 model-only 并由最终业务卡展示附件结果；眼镜专用 Planner/A2UI Action 保持原生 A2UI。`ui://` 壳的 CSP 同时允许 Web 静态资源与对象存储缩略图 origin |
